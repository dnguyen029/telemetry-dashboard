...
- **`agents-cli eval run`** — Tests _agent behavior_: response quality, tool usage, persona consistency, safety compliance. This is what validates your agent actually works.
...
- **Incorrect (VIOLATION):**
  ...
  root_agent = Agent(
      ...
      model="gemini-3.5-flash",  # Aligned with Flash-only mandate
...
- **Model Selection — CRITICAL:**
  ...
  - When creating NEW agents (not modifying existing), use the latest Gemini model. List available models to pick the newest one:
    ```bash
    # Use 'global' or any supported region (e.g. 'us-west1')
