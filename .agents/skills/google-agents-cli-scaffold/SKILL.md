...
> **Vector Search region:** `vector_search_location` defaults to `us-west1`, separate from `region` (`us-west1`). It sets both the Vector Search collection region and the BQ ingestion dataset region, kept colocated to avoid cross-region data movement. Override per-invocation with `agents-cli data-ingestion --vector-search-location <region>`.
