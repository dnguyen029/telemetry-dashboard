...
> - **Environment Alignment**: You **MUST** read the central [bootstrap.md](file:///home/dnguyen029/antigravity-project/bootstrap.md) file during your initialization/discovery phase to align with host environment specs.
...
- **Rule 10 — Automatic Knowledge Archival**: At the end of every successful task or session resolution (even in direct chat), the agent MUST automatically archive lessons learned as follows:
  ...
  2. **If Supabase is unavailable** (EOF/connection error at session end): write the lesson to [`pending_lessons.json`](file:///home/dnguyen029/antigravity-project/pending_lessons.json) in the project root. The [`flush_pending_lessons.py`](file:///home/dnguyen029/antigravity-project/.agents/hooks/flush_pending_lessons.py) PreInvocation hook will automatically retry via direct HTTP at the start of the next session.
     > Note: [`sync_context.py`](file:///home/dnguyen029/antigravity-project/.agents/hooks/sync_context.py) is the **dashboard hook** (updates `agent_live.md`) — it does NOT sync lessons. Use [`archive_lessons.py`](file:///home/dnguyen029/antigravity-project/.agents/hooks/archive_lessons.py) for Supabase archival.
...
- **Rule 12 — Environment Bootstrap Mandate**: All agents MUST read the central [bootstrap.md](file:///home/dnguyen029/antigravity-project/bootstrap.md) file and the master knowledge index [INDEX.md](file:///home/dnguyen029/antigravity-project/INDEX.md) file during their initialization/discovery phase to align themselves with host environment specifications, active virtual environments, active CLI paths, and to map the workspace architecture without guessing paths.
- **Rule 13 — Swarm Orchestration Mandate**: If the user triggers `/orchestrate` or requests orchestration, the agent **MUST** execute the multi-agent swarm flow (Orchestrator ➔ Builder ➔ Auditor ➔ SRE) exactly as defined in [orchestrate.md](file:///home/dnguyen029/antigravity-project/.agents/workflows/orchestrate.md). The agent is strictly prohibited from bypassing this sequence or executing it as a single agent based on its own judgment of "overkill".
